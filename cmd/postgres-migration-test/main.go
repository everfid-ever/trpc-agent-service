...
func verifyUp(ctx context.Context, runner *migrations.Runner, db *sql.DB, probes, repoRoot, dsn string) error {
	...
	command := exec.CommandContext(ctx, "go", "test", "-p=1", "-count=1", "./trpcservice/agentapp/postgres", "./trpcservice/config/postgres", "./trpcservice/governance/postgres", "./trpcservice/storage/artifact/postgres", "./trpcservice/storage/messaging/postgres", "./trpcservice/storage/session/postgres", "./trpcservice/tenant/postgres")
