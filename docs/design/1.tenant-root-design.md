...
生产 Worker 与 `webui-local` 两种执行角色都使用官方 `memory/postgres` Service，并显式 `WithSkipDBInit(true)`：`memories` 表由本服务 migration baseline 唯一所有。Memory 的上游 scope 使用已编码的 `tenantID/agentAppID` AppName，因而与租户隔离一致；演示角色不再以“未注入 Memory”悄然缩小已发布 Revision 的能力面。
