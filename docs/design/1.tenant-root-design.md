...
6. 静态能力默认拒绝：只有 Revision allowlist 中的 Tool/ToolSet/Skill/Knowledge 才能进入装配；动态治理仍需 PolicySnapshot 再次授权。MCP 当前未接入，不能以 ToolSet 名义绕过该 allowlist。
...
`SkillFilter` 只负责可见性，不是最终授权。Skill 声明的 Tool、Knowledge 和 Secret capability 必须同时属于 Revision allowlist，并在每次执行时经过 PolicySnapshot、参数级 Tool Guard 和 ScopedSecretProvider；Skill 文本不能扩大权限。MCP 当前未接入，Skill 不能借由描述文本声明远端 MCP 能力。Repository 加载失败、digest 漂移、scope 不匹配或引用未固定时 fail closed，不得退回共享目录或最新版本。
