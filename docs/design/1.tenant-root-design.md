...
本文统一定义控制面的两个核心领域：Tenant 是配置、数据、权限、审计与成本归属的顶层隔离根；Agent App 是租户内可独立发布、回滚和路由的应用根。两者共享 tenant-leading 约束、乐观锁、不可变发布、事务 Outbox 与审计规则，但保持独立的身份、生命周期和版本语义。
...
Agent App 结构与发布协议由本文第二部分定义；模型注册、工具授权、Channel、Backend 连接参数和 Secret 的独立结构由后续领域设计展开。Tenant 根表不得通过 JSONB 提前承载这些配置。
...
10. **密钥不落根表。** IM token、模型 API key、数据库密码及其密文均不进入 tenant；根表最多保存后续配置对象的非敏感引用。
...
|---|---|---|
| `agent_app` / `agent_app_revision` | 稳定 App 身份、不可变发布版本及 service-owned ExecutionProfile 投影 | Agent 定义、prompt、模型参数和工具清单；详见[第二部分](#第二部分-agent-app-领域模型与发布) |
...
子表落地后先建立同租户候选键，再添加约束。`agent_app` 的主键已由 Agent App 方案定义为 `(tenant_id, agent_app_id)`，不需要重复增加唯一约束：
...
默认引用保持可空。删除正在被默认引用的对象采用 `RESTRICT` 语义；Admin API 必须在同一事务中先切换或清空默认引用，再停用或删除目标对象。约束在子表 migration 完成后添加，避免建表循环依赖。
...
该表是状态领域事实，不是第二套通用 Audit Store。D 模块 Audit Relay 和 A 模块 tenant-control Relay 都消费 B 模块统一 `outbox` 中指向同一事实的事件，避免增加专用 Relay 或不一致的重复载荷。以下函数在通用 outbox migration 完成后创建：
...
长度前缀或等价 canonical encoding 消除拼接歧义，HMAC 避免泄露外部平台标识。密钥只通过 SecretProvider 获取并带版本；持久化主键仍显式包含 `(tenant_id, agent_app_id, session_id)`，不能仅依赖 HMAC 碰撞概率。
...
模型和工具 Secret 由 SecretProvider 根据 tenant/app 授权按需注入，只存在于最小作用域内存，不进入 Agent 缓存键、Envelope、日志、trace 或错误报告。
...
1. `agent_app` / `agent_app_revision`：稳定应用身份、不可变发布内容和 service-owned ExecutionProfile 投影，DDL 与发布协议见[第二部分](#第二部分-agent-app-领域模型与发布)。
2. `backend_profile`：`(tenant_id, backend_profile_id)`，后端能力与 SecretRef；必要时配套 `tenant_backend_binding`。
...
Admin API 第一阶段只暴露配置校验、发布、读取和回滚，领域写入仍由本文件定义的 Repository/受控数据库函数完成：
...
本方案采用“稳定根实体 + 不可变 Revision”：`agent_app` 保存身份、生命周期、当前发布指针和并发版本；`agent_app_revision` 保存草稿或已发布的 Agent 定义。模型客户端、Tool 实例、数据库连接、函数指针和 Secret 明文都不是领域数据，不得写入上述表、Envelope 或缓存键。

第一阶段仅允许 `agent_kind = 'llm'`。P5 在 Graph suspension/resume 上线前增加经过 schema、拓扑校验和 contract test 的 Graph Revision migration；Chain、Parallel 和 Cycle 也必须按相同原则显式扩展，不能把未知结构直接交给框架执行。
...
3. 发布后的 Revision 及其 Tool/Knowledge 子记录永久不可修改；更新配置必须创建或编辑 Draft，再发布新 Revision。
4. 回滚只切换 `current_revision`，不复制、不重编号、不修改历史 Revision 或 digest。
5. 所有影响执行行为的引用必须固定对象版本或不可变 digest；只保存可原地修改的裸 ID 不满足可复现性。
6. 静态工具权限默认拒绝：只有 Revision allowlist 中的 Tool/ToolSet/MCP/Skill 才能进入装配；动态治理仍需 PolicySnapshot 再次授权。
...
`generation_config` 和 `runtime_policy` 只承载已知、有限且无 Secret 的扩展字段。Go 发布校验必须解码到版本化具体类型、拒绝未知字段和非法数值，并将会影响行为的框架默认值物化后再计算 digest。

#### 4.3 Tool 与 Knowledge 引用
...
Model、Tool 和 Knowledge 表落地后必须增加以 `tenant_id` 开头的复合外键。后端选择属于 `ConfigSnapshot`/`BackendBinding`；若某个 Agent 对 Backend 有强制能力要求，Revision 保存版本化 capability requirement，而不是 DSN 或客户端对象。
...
基线触发器语义如下；Tool 和 Knowledge 子表复用同一 guard：
...
5. 校验 Model/Tool/Knowledge/Backend capability 引用属于同一租户且版本可用。
...
Digest 输入至少包含 agent kind/schema、Revision description、Prompt、模型 ID/version、排序后的 Tool/Knowledge 版本引用及 `required` 等授权属性、Backend capability requirement、generation config、runtime policy 和影响行为的已物化默认值。时间、actor、draft version、根表展示字段和 Secret 内容不得进入 digest。Digest 用于完整性、缓存和审计，不是签名或授权凭据。
...
| Snapshot 内容 | 框架装配 |
|---|---|
| immutable App key | LLMAgent name；稳定内部身份仍使用 AgentAppID |
| Revision description | LLMAgent description；根表 display name/description 不进入运行时 |
...
SecretRef 的 credential version 可单独参与客户端池键和失效，不把 Secret 内容加入 Bundle key。发布/回滚只产生新请求使用的新键；旧 Bundle 按 [Gateway/Worker §2.3](2.gateway-worker-design.md#23-runtime-bundle-manager)执行 retire、引用计数归零和有界关闭，不存在并行的装配器级缓存。

### 9. ExecutionEnvelope 映射

Envelope 必须直接固定 Agent Revision，不再传递含义重复的 ExecutionProfile ID/version。字段与序列化契约以 [Gateway/Worker §2.1 的权威定义](2.gateway-worker-design.md#21-executionenvelope-权威契约)为准；本领域只负责提供其中的 Tenant/App version、Revision、digest、Config version 和 Policy version。
...
- Tool/Knowledge 修改与发布并发时由父 Revision 行锁串行化。
...
1. 冻结 Go 领域类型、typed errors、Repository 和 Snapshot contract。
2. 实现 InMemory Repository 与上述 contract tests。
3. 增加 PostgreSQL migration、受控发布/回滚函数和 Outbox。
4. 在 ExecutionEnvelope 中直接实现 `agent_app_revision`，不引入第二套 Agent 定义版本。
5. 实现 ExecutionProfileResolver 和 RunOptionAssembler。
6. 接入 Gateway `PrepareDispatch`、Worker 复核、缓存失效和审计。
7. 再落地 Model/Tool/Knowledge/Backend 表的复合外键与版本引用。
8. P5 增加 Graph Revision schema/migration，通过跨 Worker suspension contract 后才允许发布 Graph App。
