...
当前仓库已有 Agent Revision 的固定 ModelProfile 引用、Model/Backend Profile 不可变 PostgreSQL Repository、严格 Provider Catalog、BackendBinding 和 ExecutionProfileSnapshot 引用，并已实现 Agent/Config 发布门禁与 scoped Secret 契约。DeepSeek 官方 OpenAI-compatible endpoint 的生产 client Factory 已接入；其他 provider、共享 credential generation 池及失效事件仍须按根 README 状态台账逐项登记，不能因 Profile 已可发布就推断所有运行时客户端装配已经完成。
