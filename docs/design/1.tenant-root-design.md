...
目标契约完整支持 `llm`、`graph`、`chain`、`parallel` 和 `cycle` 五类 Agent。实施可以分阶段启用，但持久化 schema、发布校验和运行时投影必须先定义完整；未通过对应 capability gate 的类型在发布时 fail closed，不能把未知结构直接交给框架执行，也不能以“首期仅 LLM”为由另建一次性模型。
...
6. 静态能力默认拒绝：只有 Revision allowlist 中的 Tool/ToolSet/MCP/Skill/Knowledge 才能进入装配；动态治理仍需 PolicySnapshot 再次授权。
...
#### 4.3 组合拓扑、Tool、Skill 与 Knowledge 引用
...
LLM 节点通过 `llmagent.WithSkills`、`WithSkillRepositoryProvider`、`WithSkillScopeMode` 和 `WithSkillFilter` 复用上游 Skill 装配与按需加载协议；service 使用 `skill.Repository`/`RootedRepository`/`RepositoryProvider` 作为端口，并把框架的 app/user scope 映射为可信的 tenant/app/user scope。tenant 不能直接提供 Repository 实现或任意文件系统根目录。
...
`SkillFilter` 只负责可见性，不是最终授权。Skill 声明的 Tool/MCP、Knowledge 和 Secret capability 必须同时属于 Revision allowlist，并在每次执行时经过 PolicySnapshot、参数级 Tool Guard 和 ScopedSecretProvider；Skill 文本不能扩大权限。Repository 加载失败、digest 漂移、scope 不匹配或引用未固定时 fail closed，不得退回共享目录或最新版本。
...
7. 落地 Model/Tool/Skill/Knowledge/Backend 和组合 Agent 子 Revision 的复合外键与版本引用。
8. 分类型启用 LLM/Graph/Chain/Parallel/Cycle capability；每类必须先通过构造器兼容、拓扑、治理和恢复 contract，不能修改统一 schema 另开旁路。
...
Agent App Revision 只保存 `model_profile_id + model_profile_version`。发布 Revision 时必须验证 ModelProfile 属于同租户、状态可用、schema 已注册且 digest 可重算。运行时 Resolver 按精确 version 读取，禁止用 current/latest 替换。
...
允许的 purpose 至少分为 `channel_verify`、`channel_send`、`model_call`、`tool_call` 和 `backend_connect`。Provider 必须校验 SecretRef 对 scope/purpose 的授权和版本；错误只返回稳定分类，不回显底层 KMS 文本。SecretValue 只能作为一次 Factory/协议调用的短期参数，不得写入 Snapshot、Envelope、context value、缓存键、审计或错误。
...
- provider adapter 尚未通过声明能力对应的 contract suite；
- distributed 模式试图回退到 mock、InMemory 或 current/latest Profile。

### 8. 实现状态与验收

当前仓库已有 Agent Revision 的固定 ModelProfile 引用、Model/Backend Profile 不可变 PostgreSQL Repository、严格 Provider Catalog、BackendBinding 和 ExecutionProfileSnapshot 引用，并已实现 Agent/Config 发布门禁与 scoped Secret 契约。生产 provider adapter/client Factory、credential generation 池及失效事件仍须按根 README 状态台账逐项登记，不能因 Profile 已可发布就推断运行时客户端装配已经完成。
