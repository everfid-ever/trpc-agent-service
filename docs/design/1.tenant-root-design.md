...
当前仓库已有 Agent Revision 的 ModelProfile 引用、BackendProfile 骨架表、BackendBinding 和 ExecutionProfileSnapshot 引用；完整 Profile Repository、Catalog、生产 Factory 与 scoped Secret 尚须按根 README 状态台账逐项登记，不能因字段存在就标记为 implemented。
