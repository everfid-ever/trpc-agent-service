...
当前已实现 SSE 最小持久终态回放：`TerminalEventStore` 只从共享 `TaskStore + ResultStore` 派生 `message.completed/run.completed/run.failed/run.denied`，因此 Gateway 重启不丢终态；版本化 HMAC cursor 固定 tenant/request/sequence，伪造、跨租户和未来序号均 fail closed。Handler 使用有界 replay 和节点级并发订阅上限，轮询只读共享事实且不接触 Runner/Bundle，慢写最多阻塞自身 handler goroutine；TCP 断开只结束该订阅，不写 cancel intent。durable run 创建入口、`GatewayRunnerBridge`、版本化 HMAC principal、OpenAI/tRPC-Agent invocation middleware、A2A durable TaskManager 与 `cmd/trpc-service gateway` listener/mux 组合根已完成；真实 PostgreSQL/Redis 双 Worker 验收已接入 production Gateway 的幂等提交、重建恢复、SSE replay 与 HTTP cancel，但仍需在授权测试环境执行。`run.started/progress/message.delta` 的共享低延迟 EventBus 和分布式限流留在后续治理/体验切片。
...
| AG-UI | 目标版本的公开 `server/agui` 协议、thread/run/event 转换 | AG-UI thread 映射 SessionID，run 映射 request ID；当前 `v1.11.2` 无该公共包，因此属于明确的升级 capability gate，不允许本地复制协议实现冒充复用 |
| A2A | `server/a2a` 的 Processor/TaskManager、消息转换、认证 middleware 扩展点；通过 `WithRunner` 或 Processor builder 注入 | A2A task ID 映射 request ID，task 状态读取 TaskStore，cancel 写共享 cancel intent |
...
当前 A2A 组合不实例化上游默认 MemoryTaskManager，也不把上游 Session 作为权威状态。`DurableA2ATaskManager` 直接实现公开 TaskManager：`message/send` 与 `message/stream` 进入共享 RunSubmitter，task ID 等于 durable request ID，`tasks/get` 读取 TaskStore，`tasks/cancel` 写带版本的共享 cancel intent，`tasks/resubscribe` 从 EventStore 重放。push notification 因尚无持久配置仓显式返回 unsupported；Agent Card 相应声明 `pushNotifications=false`、`stateTransitionHistory=false`。drain 仅拒绝新的 message send/stream，已有 task 的 get/cancel/resubscribe 继续可用。
...
取消传播是最优努力的：在轮询间隔或 hint 传递窗口内，已启动的模型、Tool 或外部系统调用可能已发生一次，其本地结果必须被丢弃并收敛为 `cancelled` 终态。数据库 trigger 保证 cancel intent 之后不再提交 succeeded/failed/denied 等非取消终态与 Session 业务结果，但不能撤回已发生的外部效果；外部写入仍必须依赖幂等键、最终 Tool Guard 与 Provider 侧去重。当前一个 execution 只接受一个 cancel intent，因此 `cancel_version` 实际为 1；版本化表结构仅保留给未来多阶段取消协议，当前 API 不得伪造后续版本。
