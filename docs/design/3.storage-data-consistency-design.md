...
Agent App 根表、Revision、Tool/Knowledge 引用、父行锁和发布/回滚事务的规范 DDL 见[控制面领域模型第二部分](1.tenant-root-design.md#第二部分-agent-app-领域模型与发布)。`agent_app_revision` 是 Agent 定义版本的权威来源；Storage 不得另建独立递增的 execution profile 表或版本计数器。
...
Reply event_seq 在 CommitTurn 内从本次 commit 的稳定事件次序确定性派生；同一逻辑提交重试复用相同 event_seq。continuation 使用新的 stage/ordinal，不能随机生成第二套回复序列。
...
`tenant` 根表必须先按[控制面领域模型第一部分第 5 节](1.tenant-root-design.md#5-postgresql-基线-ddl)创建；`agent_app`/`agent_app_revision` 按[第二部分](1.tenant-root-design.md#第二部分-agent-app-领域模型与发布)创建，`backend_profile` 和 `config_snapshot` 落地后再添加其余同租户复合外键。不得以仅校验单列 ID 的应用逻辑代替该约束。
