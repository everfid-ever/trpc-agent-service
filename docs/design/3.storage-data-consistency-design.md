...
Artifact 的实现边界固定为“SQL 权威元数据 + tenant-scoped immutable ObjectStore 内容”。`000018_artifact_object_store` 在不改写 `000017` 的前提下增加 `storage_kind/object_key/content_size`：历史 `postgres_bytea` 行保持可读，新对象行的 PostgreSQL `content` 必须为 NULL，逻辑 `ArtifactRef` 与后端 `object_key` 不得混用。对象必须先按稳定 key 幂等写入并复核 digest/size，再提交 SQL 元数据；SQL 失败可能留下不可见 orphan，但重试复用同一 key，不允许覆盖不同内容。对象缺失或校验不一致时读取 fail closed，分别按 backend unavailable/version mismatch 处理。生产 ObjectStore provider、保留期和基于 `media_artifact_object_lifecycle_idx` 的 orphan/retention reconciler 属 09-08 组合根与运维接线，不得由请求路径即时删除可能正被并发提交的对象。
