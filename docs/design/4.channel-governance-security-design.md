...
type Adapter interface {
    ...
    Verify(context.Context, CallbackRequest, ScopedVerifierHandle) (VerificationReceipt, error)
...
当前实现阶段只落地了 service-owned Adapter 的出站最小子集（生命周期、`Deliver`、永久/可重试/结果不确定错误分类、可选对账与 Reply Queue）；`PublicRoute`、`ScopedVerifierHandle`、`VerificationReceipt` 和 VerifiedBinding 促进链尚属入站待实现范围。在完整 Adapter contract suite 与上游公共 Channel 依赖门禁通过前，该状态仍为 `blocked`，不计 OpenClaw 兼容或真实 IM 生产验收。
...
type VerificationReceipt struct {
    CandidateToken, Purpose, ProtocolIdentityDigest string
...
`PublicRouteHint`、`CandidateBindingContext` 和验签失败事件均不具有 tenant 授权语义。公开 route、corp/app hint 或 callback payload 不能直接返回 `tenant_id`、`app_id`、`secret_ref` 或 Secret 值。`CandidateToken` 必须由服务端候选索引签发并绑定单一候选、purpose、签发/过期时间和一次性消费边界，不能由 URL/header/payload 自行拼接。
