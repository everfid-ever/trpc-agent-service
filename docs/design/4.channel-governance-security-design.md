...
当前实现阶段已落地 service-owned Adapter 的可执行公共入口与出站最小子集（`PublicRoute`、candidate、单用途 verifier、`VerifiedCallback`/`VerifiedBinding`、`Deliver`、永久/可重试/结果不确定错误分类、可选对账与 Reply Queue）；真实 Feishu/WeCom protocol、identity/session、Preprocess 仍属后续范围。在完整 Adapter contract suite 与上游公共 Channel 依赖门禁通过前，该状态仍为 `blocked`，不计 OpenClaw 兼容或真实 IM 生产验收。
