...
当前实现阶段已落地 service-owned Adapter 的可执行公共入口与出站最小子集（`PublicRoute`、candidate、单用途 verifier、`VerifiedCallback`/`VerifiedBinding`、`Deliver`、永久/可重试/结果不确定错误分类、可选对账与 Reply Queue），并已完成 tenant-scoped identity/session HMAC、持久 Preprocess lease/CAS 及严格文本入站到 `PrepareDispatch` 的公共闭环。Feishu webhook 第一轮已固定官方 Go SDK v3.10.0，完成加密事件签名/解密、token/app/时钟窗复核、文本与严格 mention 映射、Reply UUID、按 credential generation 共享 SDK Client/token cache 及 HTTP/provider code 错误分类；WeCom Agent XML 第一轮已完成 SHA-1 callback signature、AES-CBC/PKCS#7、receive-id/CorpID/AgentID/时钟窗复核、严格文本映射、公共 durable Pipeline 和应用消息 API 的 token 单次刷新与错误分类。HTTP challenge/组合根、Bot WebSocket/Webhook、媒体 staging、WeCom 共享 token provider 的具体后端和真实凭据 smoke 仍属后续范围。在完整 Adapter contract suite 与上游公共 Channel 依赖门禁通过前，该状态仍为 `blocked`，不计 OpenClaw 兼容或真实 IM 生产验收。
...
当前第一轮只实现 Agent 加密 XML 的纯文本 callback 与应用消息发送：协议层使用标准库实现并按企业微信官方 Go 示例的字段、排序、AES 布局和 XML envelope 做兼容测试；`AccessTokenProvider` 负责 scoped credential、共享缓存和强制刷新，sender 在明确的 token 失效码后最多强制刷新一次。WeCom 没有等价于 Feishu Reply UUID 的请求级幂等字段，`clientRequestID` 只用于本地 Delivery Ledger，Provider 侧启用的 `enable_duplicate_check` 是按收件人和内容在 1800 秒窗口内去重；它既弱于请求级幂等，也可能抑制窗口内合法的相同文本，启用前必须由产品接受这一平台语义。URL 验证 GET、Bot WebSocket/Webhook、群聊 mention、媒体、共享 token cache 的具体后端和 production main 接线尚未完成，不能据此宣称 WeCom 生产验收完成。
