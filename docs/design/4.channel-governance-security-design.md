...
Artifact 内容持久化不得把对象存储 key 暴露为模型可见引用。SQL 只持有 tenant/request、稳定 `ArtifactRef`、对象 key、digest/size/MIME、malware/DLP 版本和生命周期索引；对象内容由显式注入的 tenant-scoped ObjectStore 读取并再次校验。object-first 写入必须先建立 durable upload intent，元数据提交与 intent 删除同事务；orphan reconciler 只有在 intent 过期、SQL 引用复核为空且条件 digest 匹配时才能删除对象。当前 PostgreSQL bytea 仅作为滚动升级兼容与小型部署参考后端，分布式生产组合根必须注入 ObjectStore provider；真实 scanner/provider 不可用时不能回退 bytea 或跳过扫描。
