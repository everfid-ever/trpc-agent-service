...
当前实现阶段已落地 service-owned Adapter 的可执行公共入口与出站最小子集（`PublicRoute`、candidate、单用途 verifier、`VerifiedCallback`/`VerifiedBinding`、`Deliver`、永久/可重试/结果不确定错误分类、可选对账与 Reply Queue），并已完成 tenant-scoped identity/session HMAC、持久 Preprocess lease/CAS 及严格文本入站到 `PrepareDispatch` 的公共闭环。Feishu webhook 已固定官方 Go SDK v3.10.0，完成加密事件签名/解密、token/app/时钟窗复核、文本与严格 mention 映射、URL verification challenge、Reply UUID、按 credential generation 共享 SDK Client/token cache 及 HTTP/provider code 错误分类；WeCom Agent XML 已完成 SHA-1 callback signature、AES-CBC/PKCS#7、receive-id/CorpID/AgentID/时钟窗复核、URL 验证 GET、严格文本映射、公共 durable Pipeline 和应用消息 API 的 token 单次刷新与错误分类。双 IM image/file 引用、scoped 官方下载、SSRF/大小/MIME/扫描 fail-closed staging、prepared payload/ArtifactRef 执行水合及 SQL/ObjectStore 分层也已完成仓库内 fixture。统一 HTTP endpoint 仅在 challenge 验证成功或普通 callback durable Accept 成功后返回 provider ACK，并提供只接受显式依赖的可注入 `NewMux` 组合根；生产 `main` 仍等待 09-08 配置、持久 store、生命周期和 readiness 总装。Bot WebSocket/Webhook、WeCom 共享 token provider 的具体后端、真实 scanner/ObjectStore provider、对象生命周期和真实凭据 smoke 仍属后续范围。在完整 Adapter contract suite 与上游公共 Channel 依赖门禁通过前，该状态仍为 `blocked`，不计 OpenClaw 兼容或真实 IM 生产验收。
...
Artifact 内容持久化不得把对象存储 key 暴露为模型可见引用。SQL 只持有 tenant/request、稳定 `ArtifactRef`、对象 key、digest/size/MIME、malware/DLP 版本和生命周期索引；对象内容由显式注入的 tenant-scoped ObjectStore 读取并再次校验。当前 PostgreSQL bytea 仅作为滚动升级兼容与小型部署参考后端，分布式生产组合根必须注入 ObjectStore provider；真实 scanner/provider 不可用时不能回退 bytea 或跳过扫描。
