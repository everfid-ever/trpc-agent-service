...
| 上游依赖 | OpenClaw 公共 Channel 契约；B 模块 Binding、Inbox、Delivery Ledger、Artifact 与 Outbox 契约 |
...
本服务复用的是 tRPC-Agent-Go OpenClaw 的**公共 Channel 组件契约**，不是把 OpenClaw 单应用产品运行时整体嵌入多租户数据面。OpenClaw 当前公开的基座包括：

```go
// 来源：trpc-agent-go/openclaw/channel
...
service-owned `Adapter` 是该契约的严格超集，目标接口关系为：

```go
type Adapter interface {
    occhannel.Channel // 直接复用 ID 和 Run
...
var _ occhannel.Channel = (Adapter)(nil)
```

Go 接口为结构化满足关系，因此 WeCom/Feishu Adapter 只实现一套 `ID/Run`。`Deliver` 是平台可靠投递入口；若 Provider 同时实现 `TextSender`/`MessageSender`，只能由 `Deliver` 在成功 claim Delivery Ledger 后调用，Gateway、Runner 和业务代码不得直接调用 sender 绕过幂等与审计。

当前存在明确的依赖门禁：`trpc-agent-go/openclaw` 是独立 Go 1.24.1 产品模块，而本服务当前基线为 Go 1.21。落地顺序固定为：

1. 优先推动上游把 `channel`、出站值对象及必要 registry 接口抽为核心模块内、与核心 Go 基线兼容的稳定公共包；本服务直接 import 该包并增加 compile-time assertion。
2. 上游抽取完成前，本地只保留同形 `ChannelLifecycle` 兼容契约供 fake 和 contract test 使用，状态必须登记为 `blocked`，不得宣称已完成 OpenClaw 代码复用或真实 IM 生产验收。
3. 若决定直接依赖独立 `openclaw` module，必须先通过 ADR 显式批准 Go 1.24.1 升级、依赖/SBOM/许可证审查及核心模块版本收敛；只允许导入 `openclaw/channel` 等批准的公共叶子包，不允许引入 `openclaw/app`、`runtimeprofile`、`registry.ChannelDeps.Gateway` 或任何 `internal` Provider。
4. 公共契约可用后删除本地兼容声明，WeCom/Feishu 与 fake Provider 运行同一 OpenClaw compatibility suite 和 service Adapter contract suite。

不复用 `registry.ChannelDeps.Gateway` 的原因是它面向 OpenClaw 单应用 Gateway，包含 `AppName/StateDir/GatewayClient` 假设；直接使用会绕过本服务的候选验签、TenantContext、ClaimInbox、Preprocess 和 PrepareDispatch。这里复用 Channel 生命周期、出站载荷与扩展模型，入口可靠性和多租户状态机仍由本服务扩展。

当前实现阶段已落地 service-owned Adapter 的可执行公共入口与出站最小子集（`PublicRoute`、candidate、单用途 verifier、`VerifiedCallback`/`VerifiedBinding`、`Deliver`、永久/可重试/结果不确定错误分类、可选对账与 Reply Queue），并已完成 tenant-scoped identity/session HMAC、持久 Preprocess lease/CAS 及严格文本入站到 `PrepareDispatch` 的公共闭环。Feishu webhook 已固定官方 Go SDK v3.10.0，完成加密事件签名/解密、token/app/时钟窗复核、文本与严格 mention 映射、URL verification challenge、Reply UUID、按 credential generation 共享 SDK Client/token cache 及 HTTP/provider code 错误分类；WeCom Agent XML 已完成 SHA-1 callback signature、AES-CBC/PKCS#7、receive-id/CorpID/AgentID/时钟窗复核、URL 验证 GET、严格文本映射、公共 durable Pipeline 和应用消息 API 的 token 单次刷新与错误分类。双 IM image/file 引用、scoped 官方下载、SSRF/大小/MIME/扫描 fail-closed staging、prepared payload/ArtifactRef 执行水合、SQL/ObjectStore 分层、upload-intent orphan 清理和 referenced Artifact retention 也已完成仓库内 fixture；独立 Preprocess role 已接入持久 job/payload、Artifact、扫描器、`PrepareDispatch` 和双 IM 官方 media fetcher。发送凭据使用独立 `send_secret_ref/channel_send`，按冻结的 `(tenant, config_version, binding, channel, account)` 精确定位，Feishu/WeCom token cache 随 config/Secret generation 替换且不回退 current/latest。统一 HTTP endpoint 仅在 challenge 验证成功或普通 callback durable Accept 成功后返回 provider ACK，并提供只接受显式依赖的可注入 `NewMux` 组合根；`cmd/trpc-service channel` 现已将 Feishu/WeCom callback mux 和默认关闭的 WebUI endpoint 组合到 PostgreSQL ingress、tenant-scoped SecretProvider、payload key resolver、identity/session 与持久 Inbox/PreprocessJob，并把 unhealthy/draining 请求挡在 ACK 边界之前。独立 `cmd/trpc-service channel-delivery` 负责历史账号发现、Redis consume/reclaim、PostgreSQL Delivery Ledger 和 provider Adapter，callback 与 delivery 仍保持独立故障域。`webui.Adapter` 按 [M2-SD-002](8.m2-scope-decisions.md) 复用上述公共链路，以 HMAC 浏览器协议和只存元数据的 mailbox 验证本地内部语义，不建立平行 IM 体系，也不替代真实 provider 验收。真实 provider 凭据 smoke harness 与 PG/Redis Gateway→Worker→delivery runtime slice 已接入，可在授权环境显式执行；实际外部执行结果不由本地 WebUI 或 mock 代替。WeCom Bot WebSocket/Webhook、`aibot_msg_callback` 与群聊 mention 已按 [M2-SD-001](8.m2-scope-decisions.md) 正式移出 M2。OpenClaw 兼容仍受上游公开 Channel 依赖门禁约束，不把 Bot 或外部 smoke 未执行误报为已完成。
...
复用 OpenClaw Channel 不表示让 Provider 直接调用 Runner。多租户链路固定为：
...
→ ReplyEvent
→ Adapter.Deliver
→ OpenClaw TextSender / MessageSender capability
→ IM platform
```
...
Agent Event 在 Worker 内先区分可丢弃体验事件和必须持久化的终态事件。只有 CommitTurn 成功后的 Reply Outbox 可以生成可靠 `ReplyEvent`；Channel renderer 再根据 `Capabilities` 映射为纯文本、流式 edit、卡片或媒体消息。OpenClaw sender 接口是最后一跳传输能力，不承担 Session、执行终态或幂等事实。
...
M2 的协议 façade `RunSubmitter` 当前只接受纯文本，并以 `dispatch_pending` 直接进入共享 `PrepareDispatch`，不创建 IM `PreprocessJob`；这是阶段性范围边界，不是 DLP 豁免。09-07 接入 Input DLP 前，OpenAI/A2A/tRPC-Agent 等协议输入必须在 `PrepareDispatch` 前进入共享、可恢复的 Input Governance 阶段，或在协议边界提供等价的 durable guard；扫描依赖不可用或 verdict unknown 时必须 fail closed。
...
当前实现 Agent 加密 XML 的纯文本 callback、URL 验证 GET 与应用消息发送：URL 验证也必须先解析 opaque route，再在 candidate scoped verifier 内校验时钟窗、签名、AES 明文和 receive-id，成功后原样返回 challenge，不创建 Inbox。协议层使用标准库实现并按企业微信官方 Go 示例的字段、排序、AES 布局和 XML envelope 做兼容测试；`AccessTokenProvider` 以冻结 ConfigSnapshot 的独立 `send_secret_ref` 解析严格 `{corp_id,corp_secret,agent_id}`，负责共享缓存和强制刷新，sender/media fetcher 在明确的 token 失效码后最多强制刷新一次。WeCom 没有等价于 Feishu Reply UUID 的请求级幂等字段，`clientRequestID` 只用于本地 Delivery Ledger，Provider 侧启用的 `enable_duplicate_check` 是按收件人和内容在 1800 秒窗口内去重；它既弱于请求级幂等，也可能抑制窗口内合法的相同文本，启用前必须由产品接受这一平台语义。Bot WebSocket/Webhook 与群聊 mention 按 [M2-SD-001](8.m2-scope-decisions.md) 移出 M2；媒体、生产组合接线与 provider smoke harness 已实现，真实凭据 smoke 按 [M2-SD-003](8.m2-scope-decisions.md) 在 M3 授权环境执行，不能用本地 fixture 冒充 WeCom 生产验收。
...
实现映射中 `reconciling` 是逻辑阶段，不额外持久一个 Ledger state；行在 Provider 查询期间仍保持 `ambiguous`，并通过 version CAS、`reconcile_attempt` 与 `not_before` 防止并发/无界对账，查询结果再原子转为 sent/retry_wait/failed。当前出站实现明确只支持 `segment_no=0` 且 `segment_count=1`；上述多分片顺序与部分重试是后续出站轮次的必须验收项，不计入当前完成能力。
