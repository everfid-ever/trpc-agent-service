...
当前实现阶段已落地 service-owned Adapter 的可执行公共入口与出站最小子集（`PublicRoute`、candidate、单用途 verifier、`VerifiedCallback`/`VerifiedBinding`、`Deliver`、永久/可重试/结果不确定错误分类、可选对账与 Reply Queue），并已完成 tenant-scoped identity/session HMAC、持久 Preprocess lease/CAS 及严格文本入站到 `PrepareDispatch` 的公共闭环。Feishu webhook 已固定官方 Go SDK v3.10.0，完成加密事件签名/解密、token/app/时钟窗复核、文本与严格 mention 映射、URL verification challenge、Reply UUID、按 credential generation 共享 SDK Client/token cache 及 HTTP/provider code 错误分类；WeCom Agent XML 已完成 SHA-1 callback signature、AES-CBC/PKCS#7、receive-id/CorpID/AgentID/时钟窗复核、URL 验证 GET、严格文本映射、公共 durable Pipeline 和应用消息 API 的 token 单次刷新与错误分类。双 IM image/file 引用、scoped 官方下载、SSRF/大小/MIME/扫描 fail-closed staging、prepared payload/ArtifactRef 执行水合、SQL/ObjectStore 分层、upload-intent orphan 清理和 referenced Artifact retention 也已完成仓库内 fixture。统一 HTTP endpoint 仅在 challenge 验证成功或普通 callback durable Accept 成功后返回 provider ACK，并提供只接受显式依赖的可注入 `NewMux` 组合根；生产 `main` 仍等待 09-08 配置、持久 store、两个 Artifact reconciler 生命周期和 readiness 总装。Bot WebSocket/Webhook、WeCom 共享 token provider 的具体后端、真实 provider 凭据 smoke 仍属后续范围。在完整 Adapter contract suite 与上游公共 Channel 依赖门禁通过前，该状态仍为 `blocked`，不计 OpenClaw 兼容或真实 IM 生产验收。
...
`ChannelBinding` 唯一绑定 `(channel, external_account_id)` 到一个 tenant/app，包含 transport 配置和 SecretRef。公开索引只保存不可逆 route/account digest 到 opaque candidate identity 的映射；tenant/app/SecretRef 只能在 `PromoteVerified` 成功后从受控 Binding Store 读取。找不到、候选不唯一、candidate token 重放、purpose 不匹配或密钥版本不可用时均 fail closed。tenant 为 suspended/disabled 时仍可在验证成功后 durable claim callback 以完成去重和安全 ACK，但必须生成非执行终态，不能进入 PrepareDispatch；disabled 不再发送普通业务回复。状态边界以 [Tenant 根实体文档（第一部分第 6 节）](1.tenant-root-design.md) 为准。
...
当前实现 Agent 加密 XML 的纯文本 callback、URL 验证 GET 与应用消息发送：URL 验证也必须先解析 opaque route，再在 candidate scoped verifier 内校验时钟窗、签名、AES 明文和 receive-id，成功后原样返回 challenge，不创建 Inbox。协议层使用标准库实现并按企业微信官方 Go 示例的字段、排序、AES 布局和 XML envelope 做兼容测试；`AccessTokenProvider` 负责 scoped credential、共享缓存和强制刷新，sender 在明确的 token 失效码后最多强制刷新一次。WeCom 没有等价于 Feishu Reply UUID 的请求级幂等字段，`clientRequestID` 只用于本地 Delivery Ledger，Provider 侧启用的 `enable_duplicate_check` 是按收件人和内容在 1800 秒窗口内去重；它既弱于请求级幂等，也可能抑制窗口内合法的相同文本，启用前必须由产品接受这一平台语义。Bot WebSocket/Webhook、群聊 mention、媒体、共享 token cache 的具体后端、production main 接线和真实凭据 smoke 尚未完成，不能据此宣称 WeCom 生产验收完成。
...
公开入站验签不调用 tenant-scoped `Resolve`，而是通过 `CandidateBindingContext → AcquireVerifier` 获得短期单用途 `ScopedVerifierHandle`。该 handle 不可序列化、不可进入领域对象、不可交给 Runner，并在 Verify 返回前销毁。验签成功后，Adapter/Worker 才能分别使用可信 scope 调用 `channel_send`、`model_call`、`tool_call` 或 `backend_connect` purpose；purpose 或资源版本不匹配必须拒绝。
