...
> 本仓库当前按 Docker Desktop 本地验收交付。文中 Kubernetes、远端告警、容量与发布部分保留为架构设计，不对应仓库内可执行部署资产；可执行观测入口为 Compose 的 OTel Collector 与 Jaeger。
...
当前本地版只保留上述 Compose 资产。Helm、Dashboard、Alert、Runbook 与 Loadtest 目录属于原生产架构候选项，未作为本仓库交付物维护。
