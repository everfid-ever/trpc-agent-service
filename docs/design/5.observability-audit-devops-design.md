...
每个实现切片在接入业务逻辑时同步登记 operation、允许属性、metric、错误分类、脱敏负向测试和 no-op 行为；禁止到 P6 再集中补埋点。
...
依赖 readiness 不得在 HTTP 请求路径同步访问后端。进程以每依赖独立 timeout 的并发 probe 周期缓存最近结果；尚未完成首轮 probe、任一关键依赖最近失败、migration 缺失/checksum 漂移，或 Lifecycle 进入 draining/stopped 时 `/readyz` 必须返回 503，`/livez` 仍只反映进程存活。Artifact lifecycle 角色固定探测 PostgreSQL、全部 embedded migration、Redis、versioned ObjectStore、ClamAV engine version、probe tenant 的 DLP SecretProvider scope、payload encryption key scope 和带授权的 DLP health；独立 Preprocess role 探测同一 PostgreSQL/migration、ObjectStore、扫描器和 Secret scope，但不要求 Redis，并把 preprocess claim/dispatch loop 的 drain 与 Artifact reconciler 分开；Channel callback role 探测 PostgreSQL、全部 migration、只读 SecretProvider mount 和 probe tenant 的 payload key scope，并在 callback handler 前复用同一 readiness gate，不要求 Redis、ObjectStore、ClamAV 或 DLP；Channel delivery role 探测 PostgreSQL/migration、Redis、Secret mount、payload key exact generation 与历史账号 catalog，不要求 callback、ObjectStore 或 scanner，动态 binding Secret 仍在单条事件发送前 fail closed。SIGTERM/SIGINT 先切 draining，再停止本角色的新领取并有界关闭 HTTP。
...
生产 Kubernetes：2+ Gateway、3+ Worker、按账号/回调负载扩展 Channel（当前 `cmd/trpc-service channel` 同时承载 Feishu/WeCom callback，可按 provider 拆 Deployment）、独立 Relay/Preprocess/Admin、PostgreSQL HA、Redis HA、对象存储、Collector gateway。配置 NetworkPolicy、PDB、anti-affinity、workload identity 和独立 ServiceAccount。
...
完成门禁：全链 trace 可查询；审计字段/类型一致；secret canary 为零；Pod drain 和 Worker kill 无旧 fence 写入；Collector 故障不阻断业务；租户级配置能灰度和回滚。
