...
- `session.Service`（当前以 TransactionalTenantSessionService 适配接入）；`memory.Service` 与 `artifact.Service` 及其公开后端（当前不直接注入，服务自有 Store 保持权威）；`knowledge.Knowledge`、Retriever、VectorStore、Source、Embedder。
- Tool、`skill.Repository`/`RepositoryProvider`、Skill scope/load 协议；`tool/mcp` 公共 API 经 `trpcservice/tool/mcp` 适配后接入组合根——首版仅固定 HTTPS SSE/streamable 端点与固定远端工具名（`TRPC_MCP_ENDPOINTS` 装配，非法拒启），未启用 stdio、动态 URL 或 `mcpbroker`。
...
Runner 当前只注入 tenant-aware Session facade；Memory 与 Artifact 由 service-owned Store 维持租户、对象生命周期与原子提交语义，Knowledge 通过 tenant-aware adapter 连接上游 `knowledge.Knowledge`。AgentFactory 按固定 Snapshot 装配 Agent/Runner/Skill，RuntimeBundleManager 按 tenant/app/revision/digest/config/policy 构建键独占复用与生命周期所有权，负责引用计数、retire 与关闭。
...
| Memory | `memory.Service` 与公开后端 | 当前不直接接入；service-owned Store 是权威实现 | tenant scope、水位、迁移和对账 | AppName/UserID 不作为租户边界；不得把同形 Store 说成上游直接复用 |
| Artifact | `artifact.Service` 与公开后端 | 当前不直接接入；service-owned Store 是权威实现 | tenant object key、版本、扫描和生命周期 | 不接受任意外部 URL/路径直达模型或 Tool；不得把同形 Store 说成上游直接复用 |
...
| Tool / MCP | `tool`、权限 RunOptions；`tool/mcp`、`mcpbroker` 公共 API | Tool 执行协议直接复用并外包 tenant-scoped wrapper；MCP 经 `tool/mcp` 适配接入——首版仅固定 HTTPS 端点、固定远端工具名、SecretRef 注入、无通用 `mcp_call` 逃逸 | Catalog/version、allowlist、Secret 注入、参数/资源 Guard | 裸 Tool、动态/子 Agent、framework tool 均不能绕过最终 Guard；stdio、模型提供 URL、动态 MCP ToolSet 直接暴露给 Agent 一律禁止 |
