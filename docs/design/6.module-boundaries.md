...
- Tool、`skill.Repository`/`RepositoryProvider`、Skill scope/load 协议；`tool/mcp` 与 `mcpbroker` 是可用的上游公共 API，但当前不在运行时组合根中。
...
| Tool / MCP | `tool`、权限 RunOptions；`tool/mcp`、`mcpbroker` 公共 API | Tool 执行协议直接复用并外包 tenant-scoped wrapper；MCP 当前未接入 | Catalog/version、allowlist、Secret 注入、参数/资源 Guard | 裸 Tool、动态/子 Agent、framework tool 均不能绕过最终 Guard；不得把可导入的 MCP API 说成已启用能力 |
