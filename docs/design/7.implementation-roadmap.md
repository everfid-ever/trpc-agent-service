...
| 实施模式 | 单人按模块依赖顺序交付 |
| 生产基线 | PostgreSQL、Redis、两个无状态 Worker、Telegram、WeCom |
...
- 固定核心模块 revision/Go 1.21，依赖检查禁止 `openclaw` 模块和上游 `internal` import。
...
1. **依赖固定**（本仓库 go.mod 目前零依赖）：
   ...
   ```
   核对基线为正式 release tag `v1.11.2`（核心模块 Go 1.21，已核对含全部所需公共 API）。`lint.sh` 通过 `go list -m all` 和源码 import 检查禁止 `openclaw` 与上游 `internal`。
...
3. **契约符号落点**（冻结时可编译的起点，符号 → 文件 → 来源；下表代码路径均相对于 `trpcservice/`）：
   ...
   | SecretProvider | secrets/provider.go | 4 号 §10 |
...
退出：tenant 与 Agent App DDL/CAS/状态审计通过；并发 Draft/发布、失败发布保持旧 Revision、历史回滚和 published 不可变测试通过；Config rollback 生成更大新版本；Admin 未认证或 path tenant 不匹配 fail closed；相同 user/session/app 标识在两租户间完全隔离，配置回滚只影响新请求。
...
- Telegram 官方 Bot API Adapter。
- WeCom 官方 Bot/Agent API Adapter和官方 crypto fixtures。
- Preprocess Worker、媒体 Artifact、Delivery Ledger；SSE 慢消费者降级、终态持久回放和媒体 SSRF/DNS rebinding/zip bomb 防护。

退出：两协议重复入站只执行一次；Adapter/Worker kill 可恢复；回复重投不重复已确认分片。
...
- 增加经过 schema/拓扑校验的 Graph Agent App Revision 类型；未知 agent kind 继续 fail closed。
- 基于 graph checkpoint 或 external tool-call continuation 的 suspension/resume、confirmation/grant。
- SecretProvider、轮换、Redactor、secret canary。
...
- 通用 Channel Framework，以及 Telegram、WeCom 两个平级 Adapter 的最小生产链路。
- Tenant-aware Session/Memory/Artifact Router，以及 InMemory、PostgreSQL/Redis、对象存储、向量库或外部 Memory 的统一能力接口。
...
本周期以 PostgreSQL + Redis 为完整生产基线。其他厂商后端至少完成统一 capability/contract 和可运行参考适配器或测试替身，不要求在本周期完成所有厂商优化。Telegram/WeCom 优先完成文本、基础媒体、验签、去重和回复重试；高级卡片、撤回和流式体验可后续增强。任何范围降级必须在 9 月 4 日前记录原因、影响、替代验收和后续日期。
...
| 08-24 周一 | 全局/A | TenantContext、Envelope、AgentApp/Revision contract、ExecutionProfileSnapshot、RunOptionAssembler、typed errors、fake ports；验证 TurnTx 与两类 suspension 接缝 | 核心契约可编译；AgentAppRevision 是唯一 Agent 定义版本；依赖图无 openclaw/internal；上游缺口有确定实现路径 |
...
| 09-02 周三 | A | Worker reclaim、status/cancel、drain、ExecutionProfile/RuntimeBundleManager；AgentFactory 仅负责 Bundle 内装配 | 执行中 kill 后安全接管，无乱序或重复回复；Bundle retire/引用计数/有界关闭通过；复用与生命周期所有权唯一 |
| 09-03 周四 | C/B | Channel Framework、Binding/identity/session、Delivery Ledger、Preprocess、SSE/媒体安全 | fake provider contract 全通过；durable ACK 可恢复；慢订阅不阻塞提交，终态可回放，恶意 URL fail closed |
| 09-04 周五 | C | Telegram/WeCom 验签解密、文本/基础媒体、回复限频重试 | 两个平台端到端 smoke；冻结剩余范围和风险 |
| 09-07 周一 | C/A/B | PolicyChain、Graph Revision schema、上游 Tool RunOption 映射、最终 Tool Guard、DLP、预算与 PricingSnapshot、两类 continuation、confirmation/grant、SecretProvider | 未知 kind、裸 Tool 绕过和未知价格均 fail closed；framework/dynamic/sub Agent 越权 fail closed；Graph/LLM ask 可接管恢复；无明文密钥 |
...
| M2 | 09-04 | 分布式 Runtime 闭环；Telegram 和 WeCom 都完成真实最小链路 |
...
- PostgreSQL + Redis + 两 Worker + Telegram/WeCom 的可复现实例。
