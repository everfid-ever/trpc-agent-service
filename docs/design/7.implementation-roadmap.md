...
| 双 IM 最小发送能力 | 双 Adapter 共用 Delivery Ledger contract；`cmd/trpc-service channel-delivery` 已组合历史账号发现、Redis consume/reclaim、PostgreSQL Result/Ledger、冻结版本 channel_send credential 与 Feishu/WeCom official sender；mock 覆盖稳定请求 ID、token 刷新与永久/重试/ambiguous 分类 | 完成（生产组合已接线，真实凭据 smoke 待执行） |
