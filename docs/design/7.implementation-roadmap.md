...
| PostgreSQL AtomicSessionStore、Inbox/Outbox 基线 | `implemented` | 已有 migration、store 与 contract/integration test 资产；完整故障验收未因此自动完成 |
...
| Model/Backend Profile 与 Provider Catalog | `contract_frozen` | 已有引用和 BackendProfile 骨架，完整 Repository/Catalog/Factory 尚未实现 |
...
- GatewayRunnerBridge；OpenAI/A2A/tRPC-Agent 协议 façade 接入共享 Inbox/Task/Event Store，AG-UI 在目标上游版本门禁通过后接入。
...
| 09-02 周三 | A | Worker reclaim、status/cancel、drain、ExecutionProfile/RuntimeBundleManager；五类 Agent 与 Skill 确定性装配；GatewayRunnerBridge 和协议 façade | 执行中 kill 后安全接管，无乱序或重复回复；Bundle 生命周期唯一；协议入口无本地 Runner/InMemory 旁路 |
| 09-03 周四 | C/B | OpenClaw-compatible Channel Framework、Binding/identity/session、Delivery Ledger、Preprocess、SSE/媒体安全 | OpenClaw compatibility 与 service Adapter contract 全通过；durable ACK 可恢复；慢订阅不阻塞提交，终态可回放，恶意 URL fail closed |
