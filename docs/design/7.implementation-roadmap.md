...
| pre-tenant candidate/scoped verifier | `implemented` | Candidate、一次性 ScopedVerifierHandle、PromoteVerified、过期回收及 PostgreSQL/InMemory contract 已落地；真实 provider 环境验收仍单独执行 |
...
| HTTP/SSE production Gateway | `implemented` | production listener/mux、认证 run/status/cancel、持久 cursor/replay、drain/readiness 已接线；PG/Redis 多角色恢复验收已纳入 opt-in runtime slice，真实端点执行仍待授权环境 |
...
| 上游协议 Server 复用 | `implemented` | GatewayRunnerBridge、OpenAI、A2A durable TaskManager 与 tRPC-Agent façade 已接共享事实；AG-UI 等待包含公共包的目标版本能力门禁 |
...
| Channel callback production composition | `cmd/trpc-service channel` 组合 PostgreSQL ingress、migration/SecretRoot/payload-key readiness、双渠道 callback mux、draining gate 与过期 candidate reaper；不要求 Redis/S3/Scanner，不创建 InMemory fallback | 完成 |
| 双 IM 最小发送能力 | 双 Adapter 共用 Delivery Ledger contract；`cmd/trpc-service channel-delivery` 已组合历史账号发现、Redis consume/reclaim、PostgreSQL Result/Ledger、冻结版本 channel_send credential 与 Feishu/WeCom official sender；mock 覆盖稳定请求 ID、token 刷新与永久/重试/ambiguous 分类 | 完成（生产组合已接线；真实凭据 smoke 按 M2-SD-003 纳入 M3 最终验收） |
| 本地 WebUI Channel 验收 | `webui.Adapter` 复用公共 callback、identity/session、Preprocess、Reply Queue、Delivery Ledger 与加密 ResultStore；HMAC 浏览器协议、PG mailbox 和 runtime slice 覆盖重复 callback/reclaim 幂等，不建立平行 IM 体系 | 完成（替代内部链路验收，不替代真实 provider smoke） |
| 生产 Gateway/SSE | 共享 Task/Result 派生的持久终态 EventStore、tenant/request 签名 cursor、有界 SSE handler、durable run 创建入口、GatewayRunnerBridge、版本化 HMAC principal、OpenAI/tRPC-Agent façade、A2A durable TaskManager 与 `cmd/trpc-service gateway` production listener/mux 已完成；实时 EventBus、限流仍是后续增强；PG/Redis Gateway→Worker→delivery 恢复路径已纳入 opt-in runtime slice，真实端点执行待授权环境 | 部分完成 |
| 基础媒体安全 | provider-neutral Artifact staging、双 IM scoped 下载、prepared payload/ArtifactRef、ObjectStore 分层、orphan/referenced reconciler 已完成；S3-compatible versioned ObjectStore、ClamAV INSTREAM、严格 HTTP DLP adapter、共享 contract 与正反例 smoke 已落地；Preprocess role 已按冻结 config version 与独立 channel_send SecretRef 接入 Feishu/WeCom token provider 和官方 media fetcher；Artifact lifecycle role 已组合两个 reconciler、signal drain、PG/migration/Redis/S3/scanner readiness 和只读 CSI scoped SecretProvider；真实 provider smoke harness 已落地，隔离告警后端和历史 backfill 仍待后续 | 部分完成（外部 Provider 执行按 M2-SD-003 纳入 M3） |
| 外部兼容/环境 | OpenClaw 公共 Channel 叶子包仍受依赖 gate 阻塞；Feishu/WeCom 真实企业凭据入口、配置模板和单 Provider/双 Provider smoke 已就绪 | deferred to M3（等待授权企业资源） |

因此 M2 的仓库内 Channel/Runtime 收尾以 WebUI 完整闭环、Provider fixture/mock 和生产组合入口为验收证据；真实企业凭据不再阻塞 M2，但必须作为 M3 最终门禁保留可审计状态。任何发布说明都不能用 WebUI 或 mock 结果替代真实 Provider 已通过的结论。
