...
| [channel-governance-security-design.md](4.channel-governance-security-design.md) | Telegram/WeCom 如何接入，权限、确认、密钥和脱敏如何实施 |
...
| 交付物 | 规范落点 | 覆盖口径 |
|---|---|---|
| 系统架构图 | [架构 §2](0.architecture.md#2-架构原则与组件拓扑) | Gateway、Worker、Channel Adapter、Storage Adapter、Worker-local Governance/Filter（Plugin/Guardrail hooks）、Model/Tool、Telemetry 与后端拓扑 |
| 核心时序图 | [架构 §12](0.architecture.md#12-端到端交互时序) | IM callback、租户/版本固定、Runner、Model、Tool 授权与调用、Session/Memory、CommitTurn、IM 回复和 Trace 闭环 |
| 数据模型与多后端 | [Tenant DDL](1.tenant-root-design.md#5-postgresql-基线-ddl)、[Agent App DDL](1.tenant-root-design.md#4-postgresql-基线模型)、[Storage §2/§7/§14/§18](3.storage-data-consistency-design.md) | 原始要求的最小模型和生产基线 DDL；其余完整 migration 按路线图分阶段实现；Redis/SQL/向量库/对象存储能力与一致性取舍明确 |
| 风险清单 | [Observability §19](5.observability-audit-devops-design.md#19-生产风险清单) | 12 项生产风险、缓解措施和可验证验收口径 |
...
- C：IM、治理与安全，包括 Channel Framework、Telegram、WeCom、Policy、确认和密钥。
...
| Telegram 与 WeCom | channel | `channels/telegram/`、`channels/wecom/` | 公共 contract + 官方协议测试 |
