...
第一次运行项目、配置 DeepSeek API Key 或查找环境变量时，先阅读[启动与配置指南](getting-started.md)。

本地 Docker 基础设施与应用镜像说明见
...
生产启动前必须通过受控部署步骤应用完整 PostgreSQL schema。进程不会自动变更 schema；readiness 会逐项核验版本与 checksum，缺失或漂移时保持 503。
...
真实 Feishu/WeCom 验证会调用外部 API 并产生可见消息，只能对授权测试消息和测试用户执行。凭据必须分别放在权限不宽于 `0600` 的绝对路径文件中：Feishu 文件使用严格 `{"app_id":"...","app_secret":"..."}`，WeCom Agent 文件使用严格 `{"corp_id":"...","corp_secret":"...","agent_id":123}`。环境变量只携带文件路径与非密钥 locator，不得携带 Secret 值。
...
本地一键验证可使用 Compose 的 `webui` profile。先将 DeepSeek API key 单独写入被 git 忽略的 `deploy/compose/secrets/deepseek-api-key`，再执行：

```bash
docker compose -f deploy/compose/docker-compose.m2.yml --profile webui up --build
```

访问 `http://localhost:58081/webui/`，默认 Route Key/Account ID 为 `local-webui`，Channel Token 为 `local-webui-token-change-me`。本 profile 的 `webui-local` 组合根只用于隔离本地环境：它自动建立本地 schema 和控制面 fixture，并组合 Preprocess、Dispatch/Wakeup/Reply Relay、Redis Worker、Reply Queue、Delivery Ledger 和 `webui.Adapter`。更换 token 或 Secret generation 前应使用 `docker compose -f deploy/compose/docker-compose.m2.yml --profile webui down -v` 清理一次性本地数据；生产环境不得自动变更 schema、创建 fixture 或使用默认 token。

该验证用于证明 provider-neutral 内部链路、重启恢复与重复投递语义，不代表 Feishu/WeCom 的真实凭据、外部网络和 provider API 已通过。真实 provider smoke 必须按上一节在授权环境执行；能力范围见[能力边界](../design/8.capability-boundaries.md)。
...
- `/livez`：进程可运行时返回 200。
- `/readyz`：Lifecycle ready 且角色声明的 PostgreSQL、全部 schema checksum、外部 provider、Secret scope 和 payload-key exact generation 最近一次全部成功时返回 200，否则 503。Gateway 额外要求 `gateway_auth` Secret scope 和 probe tenant payload key；Artifact/Worker/Channel/Preprocess/Delivery 只检查各自声明的依赖，不把未使用的 provider 强行纳入 readiness。
