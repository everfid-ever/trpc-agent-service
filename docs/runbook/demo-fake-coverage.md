...
该入口会构建服务镜像、在空 PostgreSQL 应用嵌入式 schema migration、发布固定 Demo Tenant/App/Revision/Policy/Config，并以 `fake-deterministic-v1` 启动 `demo-server`。随后它必须通过 `/healthz`、`/readyz`，并让 `POST /v1/chat` 返回固定的 `demo: deterministic fake response`。重复运行会复用相同的控制面 ID 和已发布 revision。

## 已覆盖

- 镜像构建、PostgreSQL 连通性、schema migration 就绪与 Demo bootstrap 的幂等性；
- Provider Catalog 对 fake model 和共享 PostgreSQL backend 的约束；
- fake Model Resolver 的无网络、无凭据普通响应路径，以及 HTTP chat 的基础编解码；
