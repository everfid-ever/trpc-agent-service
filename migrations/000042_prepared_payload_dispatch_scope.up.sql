...
-- Inbox keeps the original encrypted callback payload as its immutable source
-- evidence.  A media request is dispatched with a separately durable
-- prepared payload, so accept that reference only when it is tied to this
-- exact inbox record and has reached the ready preprocess state.
