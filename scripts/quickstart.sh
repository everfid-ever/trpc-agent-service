...
run_demo() {
  ...
  echo "[1/5] Building the credential-free demo image"
  docker build --tag "$demo_image" --file "$ROOT/deploy/compose/Dockerfile" "$ROOT"
  echo "[2/5] Starting disposable PostgreSQL and Redis"
  ...
  echo "[3/5] Publishing the deterministic fake-provider demo control plane"
  ...
  echo "[4/5] Starting the fake-provider HTTP demo service"
