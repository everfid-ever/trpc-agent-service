...
RETURNING m.tenant_id,m.migration_id,m.mutation_id,m.epoch,m.agent_app_id,m.session_id,
...
RETURNING tenant_id,migration_id,mutation_id,epoch,agent_app_id,session_id,source_version,mutation_digest,
...
func scanMutation(row scanner) (sessiondriver.Mutation, error) {
	...
	err := row.Scan(&value.TenantID, &value.MigrationID, &value.MutationID, &value.Epoch,
